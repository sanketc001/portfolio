...
export const HomeScreen: React.FC = () => {
  ...
  const apps = [
    ...
    { id: 'settings', name: 'Settings', icon: Settings, color: 'bg-slate-655 text-white' },
    { id: 'github', name: 'GitHub', icon: Github, color: 'bg-slate-900 text-white', isExternal: true, url: portfolioData.profile.github },
    { id: 'linkedin', name: 'LinkedIn', icon: Linkedin, color: 'bg-blue-755 text-white', isExternal: true, url: portfolioData.profile.linkedin }
  ]
  ...
  return (
    <div
      className="h-full w-full relative flex flex-col justify-between overflow-hidden select-none bg-slate-950"
      style={{
    ...
    >
      ...
      <div className="h-9 px-4 flex items-center justify-between text-white text-[11px] font-semibold bg-black/10 backdrop-blur-[2px] z-20">
        <span>{time}</span>
      ...
      <div className="flex-1 flex flex-col px-6 pt-4 relative overflow-y-auto pb-20">
        ...
        {!activeApp && (
          <motion.div
            ...
            className="text-white mt-4 mb-6 flex flex-col items-center text-center drop-shadow-[0_2px_4px_rgba(0,0,0,0.5)]"
          >
            <h1 className="text-5xl font-bold tracking-tight font-display">{time}</h1>
          ...
          >
            <div className="relative">
              <input
                ...
                className="w-full text-xs pl-9 pr-4 py-2.5 rounded-full border border-white/10 bg-white/20 dark:bg-black/20 backdrop-blur-md text-white placeholder-white/60 focus:outline-none focus:ring-2 focus:ring-violet-500"
              />
      ...
      <AnimatePresence>
        {activeApp && (
          ...
          >
            ...
            <div className="h-14 px-4 flex items-center justify-between border-b border-slate-205 dark:border-slate-850 bg-white/70 dark:bg-slate-900/60 backdrop-blur-md flex-shrink-0 select-none">
              <button
            ...
            <div className="h-6 w-full flex items-center justify-center bg-transparent select-none">
              <div 
                onClick={() => setActiveApp(null)}
                className="w-28 h-1 rounded-full bg-slate-400 dark:bg-slate-600 opacity-60 active:scale-95 cursor-pointer"
              />
            </div>
          </motion.div>
        )}
